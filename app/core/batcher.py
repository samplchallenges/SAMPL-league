...
def generate_batches(challenge):
    group = challenge.inputbatchgroup_set.create(max_batch_size=challenge.max_batch_size)
    for is_public in (True, False):
        # TODO: each batch must share a parent as well
        elements = challenge.inputelement_set.filter(is_public=is_public, is_parent=False).order_by("name").all()
...
def batchup(batch, batch_elements):
    with tempfile.TemporaryDirectory() as temp_dir:
        for value_type in batch.batch_group.challenge.valuetype_set.filter(is_input_flag=True, on_parent_flag=False):
            batcher = BATCHERS.get(value_type.batch_method)
            if batcher is None:
                raise ValueError(f"Batch method must be set on value type {value_type.key}")
