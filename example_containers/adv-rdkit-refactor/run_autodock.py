...
def run_autodock(receptor, smiles, flex, sz_x,sz_y,sz_z, c_x,c_y,c_z, b_xmin,b_ymin,b_zmin,b_xmax,b_ymax,b_zmax, exhaustiveness, num_modes, output_dir, debug):
	...
	ligprep_pdbqt_path = "/tmp/lig-prep.pdbqt"
	...
	ligdock_pdbqt_path = f"{output_dir}/lig_dock.pdbqt"
	highscore_pdbqt_path = f"{output_dir}/best_dock.pdbqt"
	highscore_pdb_path = f"{output_dir}/best_dock.pdb" # switch to join()t
	...
	sz_x, sz_y, sz_z = boxsize
	c_x, c_y, c_z = center
	box = (sz_x, sz_y, sz_z, c_x, c_y, c_z)

	adv = Autodock(PYTHON_PATH, UTILITIES_PATH, VINA_PATH, receptor_path, box)
	...
	print(f"pdbqt {highscore_pdbqt_path}", end="")
	...
	print_debug(debug, "SCORE:  scoring highest scoring pose")
